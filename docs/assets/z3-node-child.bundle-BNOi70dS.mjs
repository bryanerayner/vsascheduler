...
// src/calendar.ts
var DEFAULT_OPEN_START = 8 * 60;
var DEFAULT_OPEN_END = 18 * 60;
...
function playingSaturdays(league) {
  const listed = league.season.openDays;
  if (listed?.length) return listed.filter((d) => d.open).map((d) => d.dateIso).sort();
...
function defaultStart(league) {
  return league.season.defaultOpenStart ?? DEFAULT_OPEN_START;
}
function defaultEnd(league) {
  return league.season.defaultOpenEnd ?? DEFAULT_OPEN_END;
}
function openHours(league, dateIso) {
  const row = (league.season.openDays || []).find((d) => d.dateIso === dateIso);
  if (row) return row.open ? { startMinutes: row.startMinutes, endMinutes: row.endMinutes } : null;
  const first = league.season.firstIso;
  const last = league.season.lastIso;
  if (!first || !last || dateIso < first || dateIso > last) return null;
  if (weekdayName(dateIso) !== "saturday") return null;
  return { startMinutes: defaultStart(league), endMinutes: defaultEnd(league) };
}
...
function uniqueSortedTimes(raw) {
  const seen = /* @__PURE__ */ new Set();
  const items = [];
  for (const t of raw) {
    ...
    seen.add(m);
    items.push({ m, t: t.trim() });
  }
  items.sort((a, b) => a.m - b.m);
  return items.map((i) => i.t);
}
function homeKickoffsFor(league, bracket) {
  const ag = league.ageGroups.find((a) => norm(a.name) === norm(bracket));
  if (ag?.homeKickoffs) return uniqueSortedTimes(ag.homeKickoffs);
  return uniqueSortedTimes(
    league.preferredSlots.filter((p) => norm(p.bracket) === norm(bracket) && p.day.toLowerCase() === "saturday").sort((a, b) => a.rank - b.rank).map((p) => p.time)
  );
}
function preferredKickCost(league, bracket, minutes, day = "saturday") {
  const named2 = day.toLowerCase() === "saturday" ? homeKickoffsFor(league, bracket) : [];
  if (day.toLowerCase() === "saturday" && named2.length === 0) {
    const ag = league.ageGroups.find((a) => norm(a.name) === norm(bracket));
    if (ag?.homeKickoffs) return 0;
  }
  const hits = league.preferredSlots.filter((p) => norm(p.bracket) === norm(bracket) && String(p.time || "").trim()).sort((a, b) => a.rank - b.rank);
  if (!hits.length && !named2.length) return 0;
  const sameDay = named2.length && day.toLowerCase() === "saturday" ? named2.map((time, i) => ({ time, rank: i + 1, day: "Saturday", bracket })) : hits.filter((p) => p.day.toLowerCase() === day.toLowerCase());
  const exact = sameDay.find((p) => minutesOf(p.time, -1) === minutes);
  if (exact) return (exact.rank - 1) * 8;
  if (!sameDay.length) return 80;
  if (sameDay.length >= 2) return 100;
...
function fieldOpen(league, field2, dayName, kick, vacate, dateIso) {
  const hours = openHours(league, dateIso);
  if (!hours) return false;
  if (kick < hours.startMinutes || vacate > hours.endMinutes + 5) return false;
  const win = field2.windows.find((w) => w.day === dayName);
  if (win && (kick < win.startMinutes || vacate > win.endMinutes + 5)) return false;
  return true;
}
  ...
  for (let di = 0; di < dates.length; di++) {
    ...
    for (let rawSi = 0; rawSi < use.length; rawSi++) {
      ...
      for (let fi = 0; fi < job.fields.length; fi++) {
        ...
        if (!fieldOpen(league, field2, day, kick, occ.vacate, iso2)) continue;
