...
function grassRank(league, bracket, kickoff, day) {
  const minutes = clockOr(kickoff, 9 * 60);
  const hits = league.preferredSlots.filter((p) => norm(p.bracket) === norm(bracket) && p.day.toLowerCase() === day.toLowerCase()).sort((a, b) => a.rank - b.rank);
  const hit = hits.find((p) => clockOr(p.time, -1) === minutes);
  if (!hit) return 40;
  return (hit.rank - 1) * 8;
}
function fieldOpenAt(field, dayName, kick, vacate) {
  const win = field.windows.find((w) => w.day === dayName);
  if (field.windows.length && !win) return false;
  if (win && (kick < win.startMinutes || vacate > win.endMinutes + 5)) return false;
  return true;
}
function legalFieldFor(league, game, field) {
  const ag = league.ageGroups.find((a) => norm(a.name) === norm(game.ageGroup));
  if (!ag) return true;
  return ag.fieldClasses.some((c) => norm(c) === norm(field.type));
}
...
function coachTravelHard(league, games) {
  ...
  for (const list of byCoach.values()) {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      const ready = clockOr(prev.vacate, 0) + (norm(prev.campus) === norm(next.campus) ? 0 : travel);
      if (ready > clockOr(next.warmupStart, clockOr(next.kickoff, 0) - 15)) n += 1;
    }
  }
  return n;
}
...
function scoreSeasonZ3(league, games) {
  ...
  for (const g of games) {
    ...
    row.games += 1;
    const field = findField(league, g.field);
    const day = weekdayName(g.dateIso);
    const kick = clockOr(g.kickoff, 10 * 60);
    const vacate = clockOr(g.vacate, kick + 60);
    if (field && !legalFieldFor(league, g, field)) {
      hard += HARD;
      row.hard += HARD;
    }
    if (field && !fieldOpenAt(field, day, kick, vacate)) {
      hard += HARD;
      row.hard += HARD;
    }
...
function judgePerson(league, name, mine) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    const prev = sorted[i - 1];
    const duty = sorted[i];
    if (duty.warmup < prev.vacate) {
      const sameSpot = norm(duty.field) === norm(prev.field) && norm(duty.campus) === norm(prev.campus);
      if (!sameSpot) {
        ...
        traces.push({
          ...
          headline: text,
          steps: [
            `${prev.field} occupies ${name} through ${formatClockShort(prev.vacate)}.`,
            `${duty.field} asks for a ${formatClockShort(duty.kickoff)} kickoff.`
          ]
        });
      }
    }
    const gap = duty.warmup - prev.vacate;
    const travel = campusTravel(league, prev.campus, duty.campus);
    if (norm(prev.campus) !== norm(duty.campus) && travel > Math.max(0, gap)) {
      ...
      traces.push({
        ...
        steps: [
          ...
          `Warm-up at ${duty.campus} is ${formatClockShort(duty.warmup)}.`,
...
function personClash(a, b, travel) {
  if (norm(a.campus) === norm(b.campus)) return windowsOverlap2(a, b);
  return !(a.vacate + travel <= b.warmup || b.vacate + travel <= a.warmup);
}
function fieldOpen(field, dayName, kick, vacate) {
  const win = field.windows.find((w) => w.day === dayName);
  if (field.windows.length && !win) return false;
  if (win && (kick < win.startMinutes || vacate > win.endMinutes + 5)) return false;
  return true;
...
function saturdaySlots(league, bracket) {
  const set = /* @__PURE__ */ new Set();
  const prefs = league.preferredSlots.filter((p) => p.day.toLowerCase() === "saturday");
  const mine = bracket ? prefs.filter((p) => norm(p.bracket) === norm(bracket)) : prefs;
  for (const p of mine.length ? mine : prefs) set.add(clockOr2(p.time, 9 * 60));
  for (let m = 8 * 60; m <= 16 * 60; m += 60) set.add(m);
  if (bracket) {
    for (const p of mine) {
      for (const d of [-30, 30, 90]) {
        const t = clockOr2(p.time, 9 * 60) + d;
...
function slotRank(league, bracket, minutes, day = "saturday") {
  const hits = league.preferredSlots.filter((p) => norm(p.bracket) === norm(bracket) && p.day.toLowerCase() === day.toLowerCase()).sort((a, b) => a.rank - b.rank);
  const hit = hits.find((p) => clockOr2(p.time, -1) === minutes);
  if (!hit) return 40;
  return (hit.rank - 1) * 8;
}
...
function jobDates(league, job) {
  ...
  const out = [{ iso: job.dateIso, day: sat }];
  if (!league.carveOuts.some((c) => c.mustUse)) return out;
  for (const p of league.preferredSlots) {
    if (norm(p.bracket) !== norm(job.bracket)) continue;
    const day = p.day.toLowerCase();
    if (day === sat) continue;
...
function fiatCampusCost(league, job, field, dateIso) {
  ...
  const homeDate = isHomeGuestDate(league, dateIso);
  const weTravel = awayDate || /we travel/i.test(van.guestTravel || "") && !homeDate;
  const atGuest = guestCampus && norm(field.campus) === norm(guestCampus);
  const atVan = /van/i.test(field.campus);
  if (weTravel) {
    if (atGuest) return 0;
    if (atVan) return 14;
    return 8;
  ...
  for (let di = 0; di < dates.length; di++) {
    ...
    for (let rawSi = 0; rawSi < use.length; rawSi++) {
      ...
      for (let fi = 0; fi < job.fields.length; fi++) {
        ...
        if (!fieldOpen(field, day, kick, occ.vacate)) continue;
        const venueHit = dayVenue ? norm(field.name) === norm(dayVenue) || norm(field.name).includes(norm(dayVenue)) : true;
        out.push({
          ...
          occ,
          field,
          cost: slotRank(league, job.bracket, kick, day) + fi + (dayVenue && !venueHit ? 6 : 0) + fiatCampusCost(league, job, field, iso2)
        });
      }
    }
  }
...
async function solveLeague(league, name = "Configuration 1 \u2014 First pass \u2014 no carve-outs", loadZ3 = defaultLoadZ3) {
  ...
  for (const gs of coachPairs.values()) {
    ...
    let ok = true;
    for (let i = 1; i < sorted.length; i++) {
      if ((parseClock(sorted[i].warmupStart) || 0) < (parseClock(sorted[i - 1].vacate) || 0)) ok = false;
    }
    if (ok) kept += 1;
  }
  if (pairN) {
